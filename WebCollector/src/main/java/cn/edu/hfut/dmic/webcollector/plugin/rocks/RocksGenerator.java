...
import cn.edu.hfut.dmic.webcollector.plugin.berkeley.BerkeleyDBUtils;
...
public class RocksGenerator extends Generator {
    ...
    public RocksGenerator(String crawlPath) throws RocksDBException {
        ...
        rocksIterator = crawldbDatabase.newIterator();
    ...
    public CrawlDatum nextWithoutFilter() throws Exception {
        ...
        if(rocksIterator.isValid()){
            CrawlDatum datum = RocksDBUtils.createCrawlDatum(rocksIterator.key(), rocksIterator.value());
            rocksIterator.next();
            return datum;
        }else{
            return null;
